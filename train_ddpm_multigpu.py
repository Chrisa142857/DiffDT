...
from transformers import CLIPTokenizer, CLIPTextConfig, CLIPTextModel, Qwen3ForCausalLM, get_scheduler
...
def train(args):
    ...
    for epoch_idx in range(num_epochs):
        ...
        for data in data_loader:
            ...
            # Sample random noise
            im = im[:, torch.tril_indices(N, N, -1)[0], torch.tril_indices(N, N, -1)[1]][:, None]
            noise = torch.randn_like(im).to(accelerator.device)
            
            # Sample timestep
            t = torch.randint(0, diffusion_config['num_timesteps'], (im.shape[0],)).to(accelerator.device)
        ...
        accelerator.print(f'{datetime.now()} | '+'Epoch:{} | LR: {:.7f} | Loss : {:.4f} | Val loss: {:.4f}'.format(
        ...
        if global_avg_val_loss <= best_loss:
            ...
            if accelerator.is_main_process:
                best_mweight = {k: v.detach().cpu() for k, v in unwrapped_model.state_dict().items()}
                if save_gap >= save_inter:
                    save_gap = 0
