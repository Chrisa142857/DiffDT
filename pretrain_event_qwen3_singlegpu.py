...
def main():
    device = sys.argv[1]
    # Define output path
    OUTPUT_DIR = "./icd10_tokenizer/qwen-icd10"
    # --- 1. CONFIGURATION ---
    ...
    dataset = ICDDataset("icd10_train_split.txt")
    ...
    for epoch in range(num_epochs):
        ...
        if avg_val_loss <= best_loss:
            ...
            # Only main process saves to avoid file corruption
            best_model_state = {
        ...
        print(f"[{datetime.now()}] Epoch {epoch+1} | LR: {lr_scheduler.get_last_lr()[0]:.6f} | Train: {avg_train_loss:.5f} | Val: {avg_val_loss:.5f}")
        # break
